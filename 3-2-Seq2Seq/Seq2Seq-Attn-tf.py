...
def make_batch(sentences):
    input_batch = [np.eye(vocab_size)[[word2index[i]
                                       for i in sentences[0].split()]]]
    output_batch = [np.eye(vocab_size)[[word2index[i]
                                        for i in sentences[1].split()]]]
    target_batch = [[word2index[i] for i in sentences[2].split()]]
    return input_batch, output_batch, target_batch


# [batch_size, n_step, vocab_size]
enc_inputs = tf.placeholder(tf.float32, [None, None, vocab_size])
# [batch_size, n_step, vocab_size]
dec_inputs = tf.placeholder(tf.float32, [None, None, vocab_size])
# [batch_size, n_step]
targets = tf.placeholder(tf.int64, [1, n_step])

attn = tf.Variable(tf.random_normal([n_hidden, n_hidden]))
out = tf.Variable(tf.random_normal([n_hidden * 2, vocab_size]))

# enc_output: [n_step, n_hidden]
# score: [n_hidden]
...
def get_attn_weight(dec_output, enc_outputs):
    '''
    enc_outputs: [batch_size, n_step, n_hidden]
    '''
    attn_scores = []
...
with tf.variable_scope('encode'):
    ...
    # enc_outputs: [batch_size, n_step, n_hidden]
    # enc_hidden: [batch_size, n_hidden]
    enc_outputs, enc_hidden = tf.nn.dynamic_rnn(
        enc_cell, enc_inputs, dtype=tf.float32)

with tf.variable_scope('decode'):
    dec_cell = tf.nn.rnn_cell.BasicRNNCell(n_hidden)
    dec_cell = tf.nn.rnn_cell.DropoutWrapper(dec_cell, output_keep_prob=0.5)
    # inputs: [n_step, batch_size, n_hidden]
    inputs = tf.transpose(dec_inputs, [1, 0, 2])
    hidden = enc_hidden
    for i in range(n_step):
        dec_output, hidden = tf.nn.dynamic_rnn(dec_cell, tf.expand_dims(
            inputs[i], 1), initial_state=hidden, dtype=tf.float32, time_major=True)
        attn_weights = get_attn_weight(dec_output, enc_outputs)
        Attention.append(tf.squeeze(attn_weights))
        # context: [1, 1, n_hidden]
...
trained_attn = tf.stack(Attention, 0)
...
with tf.Session() as sess:
    ...
    predict_batch = [np.eye(vocab_size)[[word2index[n]
                                         for n in 'P P P P P'.split()]]]
    result = sess.run(prediction, feed_dict={
                      enc_inputs: input_batch, dec_inputs: predict_batch})
    print(sentences[0].split(), '->', [index2word[n] for n in result[0]])
